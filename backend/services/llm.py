...
def get_openrouter_fallback(model_name: str) -> Optional[str]:
    ...
    if "claude" in model_name.lower() or "anthropic" in model_name.lower():
        ...
        # Never use OpenRouter for Google models if we have a Google API key
        if os.getenv("GOOGLE_API_KEY"):
            return None
        # Only fallback to OpenRouter if no Google API key
...
def prepare_params(
    ...
    if model_name.startswith("xai/") or "grok" in model_name.lower():
        logger.debug(f"Preparing xAI parameters for model: {model_name}")
        # Ensure XAI models use the direct XAI API key
        if os.getenv("XAI_API_KEY"):
            params["api_key"] = os.getenv("XAI_API_KEY")
            logger.debug("Using direct XAI API key")
        else:
            logger.warning("No XAI API key found, may fall back to OpenRouter")
    
    # Add Google-specific parameters
    if model_name.startswith("google/") or "gemini" in model_name.lower():
        logger.debug(f"Preparing Google parameters for model: {model_name}")
        # Ensure Google models use the direct Google API, not OpenRouter
        if os.getenv("GOOGLE_API_KEY"):
            params["api_key"] = os.getenv("GOOGLE_API_KEY")
            logger.debug("Using direct Google API key")
        else:
            logger.warning("No Google API key found, may fall back to OpenRouter")
